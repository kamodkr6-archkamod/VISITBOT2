...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = ("👁️ <b>KAMOD VISIT BOT</b> 👁️\n\nThis bot is dedicated for <b>Auto Visits</b>.\nUse: <code>/visit IND &lt;UID&gt;</code>")
...
async def visit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not allowed:
        return await update.message.reply_text(f"⛔ <b>DAILY LIMIT REACHED!</b>\n\nYou have used {count}/{DAILY_LIMIT} visits today.\nTry again after 24 hours.", parse_mode='HTML')
...
async def check_visit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=100)) as session:
        ...
        results = await asyncio.gather(*tasks)
    await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=msg.message_id, text=f"✅ Working: {results.count(200)}\n❌ Dead: {len(MEM_TOKENS) - results.count(200)}")
...
if __name__ == '__main__':
    ...
    # 🔥 High Timeout for Koyeb Stability
    request = HTTPXRequest(connect_timeout=180, read_timeout=180, write_timeout=180, pool_timeout=180)
    ...
    print("🚀 KAMOD BOT B (KOYEB + FLASK) STARTED")
    app.run_polling()
